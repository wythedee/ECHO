...
class V4(nn.Module):
    def __init__(self, channels, dim=32, pool1=4, pool2=4):
        super().__init__()
        self.cnn = nn.Sequential(
            nn.Conv2d(1, dim // 4, (1, 15), padding=(0, 2)),
            ...
            nn.Conv2d(dim // 4, dim // 2, (1, 7), padding=(0, 3)),
            nn.BatchNorm2d(dim // 2),
            nn.GELU(),
            nn.MaxPool2d((1, pool2), stride=(1, pool2)),

            nn.Conv2d(dim // 2, dim, (1, 7), padding=(0, 3)),
            nn.BatchNorm2d(dim),
            nn.GELU(),
        )
