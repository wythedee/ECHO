...
ch75 = {
    ...
    "PO9": ["PO9","I1","PPO9h","PPO9","POO9","CB1"],
    "PO10":["PO10","I2","PPO10h","POO10","CB2"],
    "P9":  ["P9","TPP9h"],
...
class AdaptiveGrouping:
    ...
    def map_trial(self, X, ch_names):
        ...
        for x, ch in zip(X, ch_names):
            idx = self.template_channels.index(self.lookup_template_ch_by_src[ch.lower()])
            newX[idx] += x
            num[idx] += 1
        return newX / num[:, np.newaxis]

    def map_to_template(self, X, ch_names):
        # Map multiple trials to the template
